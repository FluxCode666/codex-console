...
function initEventListeners() {
    ...
    document.getElementById('batch-upload-fluxcode-item').addEventListener('click', (e) => { e.preventDefault(); uploadMenu.classList.remove('active'); handleBatchUploadFluxCode(); });
...
async function uploadAccount(id) {
    const targets = [
        ...
        { label: '⚡ 上传到 FluxCode', value: 'fluxcode' },
    ];
    ...
    if (choice === 'fluxcode') return uploadToFluxCode(id);
}
...
function selectFluxCodeService() {
    return new Promise(async (resolve) => {
        const modal = document.getElementById('fluxcode-service-modal');
        const listEl = document.getElementById('fluxcode-service-list');
        const closeBtn = document.getElementById('close-fluxcode-modal');
        const cancelBtn = document.getElementById('cancel-fluxcode-modal-btn');
        const autoBtn = document.getElementById('fluxcode-use-auto-btn');
        ...
        let services = [];
        try {
            services = await api.get('/fluxcode-services?enabled=true');
        } catch (e) {
            services = [];
        }

        if (services.length === 0) {
            listEl.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:12px;">暂无已启用的 FluxCode 服务，将自动选择第一个</div>';
        } else {
            listEl.innerHTML = services.map(s => `
                <div class="fluxcode-service-item" data-id="${s.id}" style="
                    padding: 10px 14px;
                    border: 1px solid var(--border);
                    border-radius: 8px;
                    cursor: pointer;
            ...
            listEl.querySelectorAll('.fluxcode-service-item').forEach(item => {
                item.addEventListener('mouseenter', () => item.style.background = 'var(--surface-hover)');
                item.addEventListener('mouseleave', () => item.style.background = '');
                item.addEventListener('click', () => {
                    cleanup();
...
// 上传单账号到 FluxCode
async function uploadToFluxCode(id) {
    const choice = await selectFluxCodeService();
    if (choice === null) return;
    try {
        toast.info('正在上传到 FluxCode...');
        const payload = {};
        if (choice.service_id != null) payload.service_id = choice.service_id;
        const result = await api.post(`/accounts/${id}/upload-fluxcode`, payload);
        if (result.success) {
            toast.success('上传成功');
        } else {
            toast.error('上传失败: ' + (result.message || result.error || '未知错误'));
        }
    } catch (e) {
        toast.error('上传失败: ' + e.message);
    }
...
// 批量上传到 FluxCode
async function handleBatchUploadFluxCode() {
    const count = getEffectiveCount();
    if (count === 0) return;
    ...
    const confirmed = await confirm(`确定要将选中的 ${count} 个账号上传到 FluxCode 吗？\n（代理 ID 将按轮询方式分配）`);
    if (!confirmed) return;
    ...
    try {
        const payload = buildBatchPayload();
        if (choice.service_id != null) payload.service_id = choice.service_id;
        const result = await api.post('/accounts/batch-upload-fluxcode', payload);
        let message = `成功: ${result.success_count}`;
        if (result.failed_count > 0) message += `, 失败: ${result.failed_count}`;
        if (result.skipped_count > 0) message += `, 跳过: ${result.skipped_count}`;
        toast.success(message);
